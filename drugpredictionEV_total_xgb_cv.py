...
# %%
# the number of cases with EV A and EV B
report_info = dataset['drug_eva'] + dataset['drug_evb']
print(max(report_info))
print('Total:', len(dataset))
print('EV A-B:', sum((dataset['drug_eva']+dataset['drug_evb'])>0))
plt.hist(report_info, bins=list(range(16)), color='grey', alpha=0.5)
plt.show()
...
# %%
# model trainining with cross-validation
...
for i, (train, val) in enumerate(skf.split(X_trainval, y_trainval)):
    ...
    with open('./result_total_cv.txt', mode='a') as f:
        f.write(f'Fold: {i}\n')
    ...
    pickle.dump(final_model, open(f'./xgb_total_cv_{i}.pkl', 'wb'))
...
plt.savefig(f'./xgb_roc_total_cv.pdf', dpi=300, bbox_inches='tight')
plt.show()
